...
To understand the advantage with onion architecture over n-layer, we have to understand the issues with n-layer architecutre. As we can see from above diagram, in N-Layer architecture, everything is havily depend on database. It means database is the core of the architecutre. It's a great issue when we want to develop a scalable application for real world business. Becasue we have to develope an application which has to focus on the business domain to automate the business. It means business domain will be core of our application. And obhously in future we may want to change our database or other's technologoy. As a result we have to develop our application in such a way that, we  can change technology like database or user interface whenever needed without touching business domain core. That's the reason why modern architecutre are highly depended on [DDD (Domain-driven design)](https://en.wikipedia.org/wiki/Domain-driven_design). Onion architecutre is also based on DDD. 
<br>
Well, well, well
<br>
## Why microservice?
...
First we are creating an order with two item and publishing a message to OrderQueue
